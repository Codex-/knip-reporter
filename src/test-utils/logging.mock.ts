...
interface MockedLoggingFunctions {
  ...
  assertOnlyCalled: (...coreLogMocks: Array<MockInstance<(message: string) => void>>) => void;
...
export function mockLoggingFunctions(): MockedLoggingFunctions {
  ...
  const coreInfoLogMock: MockInstance<typeof core.info> = vi
    .spyOn(core, "info")
    .mockImplementation(() => undefined);
  const coreWarningLogMock: MockInstance<typeof core.warning> = vi.spyOn(core, "warning");
...
function assertOnlyCalledInner(
  ...
  if (coreLogMocks.length <= 0) {
    throw new Error("assertOnlyCalled must be called with at least one mock to assert");
  }
